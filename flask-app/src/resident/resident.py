...
def get_residents():
    json_resident = execute_command('select * from resident')
    for resident in json_resident:
        json_resident = execute_command('select * from residentWHERE username="{0}"'.format(resident['username']))
        resident.update(json_resident[0])
